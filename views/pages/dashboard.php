<div class="flex flex-col h-screen justify-between w-full bg-gradient-to-tr from-slate-100 via-teal-50 to-cyan-50 landing_page">
    <section class="navigation_bar_section sticky top-0 z-50">
        <nav class="bg-transparent w-full py-4 px-4 flex items-center">
            ...
            <div class="ml-auto float-right space-x-3 text-sm">
                ...
                <span class="inline-block prevent-select"><p class="text-cyan-500  hover:scale-110 rounded transition duration-100 ease-in-out cursor-pointer"><?= $_SESSION['username'] ?></p></span>
            </div>
        </nav>
    </section>
    ...
    <section class="w-full h-full  bg-gradient-to-tr from-slate-300 via-teal-50 to-cyan-50">    
        <div class=" h-full w-full px-5 md:px-10 py-5">
            <div class="w-full">
                <p class="font-bold md:text-4xl text-2xl text-teal-600 "><?= (date('H:i')>='05:00' && date('H:i')<'12:00'?'Good Morning':(date('H:i')>='12:00' && date('H:i')<'18:00'?'Good Afternoon':'Good Evening')) ?>  <span class="text-teal-700 md:text-2xl text-xl"><?= $_SESSION['username']?>	&#128075;</span> </p> 
